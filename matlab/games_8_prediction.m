...
% 
% currentGames_player = [2, 2, 2, 2, 2, 2, 2, 2];   % Current games of player
...
% For player
logicalMask = all(dataMatrix_player(:,1:8) == currentGames_player, 2);
...
logicalMask = all(dataMatrix_opponent(:,1:8) == currentGames_opponent, 2);
ind = find(logicalMask);
totalResult = 0;
for i=1:1:length(ind)
